...
[风格]+ [格数]+[基本信息]+[文本]-identifer+1首先请通过以下步骤来获取信息。 
--规则-- 
1.每张图片等比，严格按照格数生成。
2.图片对话内容为英文，每条对话不超过10个词。
3.漫画分镜要多变、流畅，如画面的局部、全局，人物的不同角度，保持画面节奏、刻画重点场景。
4.适当在周围加一些漫画字符，使其更加丰富。
5.Identifer部分：0001,0002,0003,0004。请根据个数来决定Identifer的值。比如1格漫画，Identifer为0001，2格漫画，Identifer为0001,0002。
风格：""" + \
            styleDescription
...
class AmericanStyleComicAgent(ComicAgent):
    def __init__(self):
        super().__init__("""
1.**视觉冲击力强**：使用清晰的线条和鲜艳的色彩，增强画面的吸引力。
2.**现代波普艺术风格**：融合点状网格和粗黑边框，展示波普艺术的现代感。
3.**对比鲜明的色彩**：采用红色、黄色、黑色和白色等色彩，增强视觉层次感。
4.**夸张的表情和动作**：人物表情和动作夸张，有效强化情感表达。
5.**手绘线条**：人物轮廓和细节通过粗犷的手绘线条展现，增加自然和艺术感。""")
    def getName(self):
        return "ComicAgent"
...
class KoreanStyleComicAgent(ComicAgent):
    def __init__(self):
        super().__init__("""
1. **细致优美的线条艺术**：线条流畅且精准。   
2. **漂亮且富有表现力的人物形象**：拥有大而传神的眼睛。 
3. **丰富的面部表情**：生动地传达情感。  
4. **动态的动作场景**：带有强烈的运动线和戏剧效果。   
5. **优雅现代的背景细节**：拥有现代感美学。""")
